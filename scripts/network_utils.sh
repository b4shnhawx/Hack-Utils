...
programs_array=(ping nmcli traceroute telnet iftop iptraf-ng nethogs slurm tcptrack vnstat bwm-ng bmon ifstat speedometer openvpn nmap tshark sipcalc nload speedtest-cli lynx elinks)#!/bin/bash#!/bin/bash#!/bin/bash#!/bin/bash
...
programs_array=(ping nmcli traceroute telnet iftop iptraf-ng nethogs slurm tcptrack vnstat bwm-ng bmon ifstat speedometer openvpn nmap tshark sipcalc nload speedtest-cli lynx elinks macchanger torctl)
...
#				then
			...
			anon)
				...
				do
					...
					echo -en " " $BLUE"1"$END")" "IP anonymizer (tor) (activate / deactivate): "$RED; torctl status | grep -ow "inactive"; torctl status | grep -ow "active"; echo -en $END
					echo -e " " $BLUE"2"$END")" "Change MAC address"
					echo ""
					echo -e " " $BLUE"0"$END")" "Cancel"
					echo ""
					echo ""
					...
					case $option in
						1)
							...
							if [[ $torctlstatus == 'tor service is: inactive' ]];
							then
								clear
								echo -e $GREEN"\n------------- Activating TOR -------------\n"$END
								torctl start
